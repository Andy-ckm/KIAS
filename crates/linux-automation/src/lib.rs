...
pub struct LinuxAutomation {
    #[allow(dead_code)] // Stored for future use in engine operations
    ...
    pub async fn execute_task(&self, task: AutomationTask) -> Result<AutomationResult> {
        ...
        let result = match &task.task_type {
            ...
            TaskType::NetworkOps { hosts, action } => {
                ...
                let summary = format!("网络操作 {} 台主机, {} 错误", hosts_count, errors_count);
                AutomationResult {
                    task_id,
            ...
            TaskType::ServiceOps { hosts, action } => {
                let hosts_count = hosts.len();
                let mut all_results = Vec::new();
                for host in hosts {
                    let result = service_manager::ServiceManager::execute(
                        &self.executor,
                        host,
                        action,
                        &self.audit,
                ...
                let errors_count: usize = all_results.iter().map(|r| r.errors.len()).sum();
                let summary = format!("服务管理 {} 台主机, {} 错误", hosts_count, errors_count);
                AutomationResult {
                    task_id,
            ...
            TaskType::ProcessOps { hosts, action } => {
                let hosts_count = hosts.len();
                let mut all_results = Vec::new();
                for host in hosts {
                    let result = process_manager::ProcessManager::execute(
                        &self.executor,
                        host,
                        action,
                        &self.audit,
                ...
                let errors_count: usize = all_results.iter().map(|r| r.errors.len()).sum();
                let summary = format!("进程管理 {} 台主机, {} 错误", hosts_count, errors_count);
                AutomationResult {
                    task_id,
