...
fn main() {
    ...
    // 提交审批
    manager.submit_for_review(&lims_change.id, "zhang.it", None, None).unwrap();
    println!("  → 已提交审批");

    // 添加审批人
    manager.add_approver(&lims_change.id, "qa.head".to_string(), "QA主管".to_string(), "QA审批".to_string()).unwrap();
    manager.add_approver(&lims_change.id, "it.manager".to_string(), "IT经理".to_string(), "IT审批".to_string()).unwrap();
    ...
    manager.approve_change(&lims_change.id, "qa.head", Decision::Approved, qa_sig, None, None).unwrap();
    println!("  → QA主管已审批");
    ...
    manager.approve_change(&lims_change.id, "it.manager", Decision::Approved, it_sig, None, None).unwrap();
    println!("  → IT经理已审批");
    println!("  → 变更已批准\n");

    // 实施变更
    manager.implement_change(&lims_change.id, "zhang.it", None, None).unwrap();
    println!("  → 开始实施...");
    manager.complete_implementation(&lims_change.id, "zhang.it", None, None).unwrap();
    println!("  → 实施完成");

    // 验证
    manager.verify_change(&lims_change.id, "qa.tester", None, None).unwrap();
    println!("  → 开始验证...");
    manager.complete_verification(&lims_change.id, "qa.tester", None, None).unwrap();
    println!("  → 验证通过");

    // 关闭
    manager.close_change(&lims_change.id, "zhang.it", None, None).unwrap();
    println!("  → 变更已关闭\n");
    ...
    // 紧急实施（跳过审批）
    manager.submit_for_review(&security_change.id, "ops.lead", None, None).unwrap();
