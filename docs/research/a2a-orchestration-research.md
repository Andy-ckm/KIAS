...
**AgentGuard 启示**: A2A 是 Google 推出的 Agent 互操作标准，AgentGuard 已有 `a2a_router.rs`，应对接 A2A 协议标准。
...
- **swarm** 的轻量级编排适合 AgentGuard 的调度场景
...
**AgentGuard 启示**: AgentGuard 的 `scheduler` + `AgentShell` 已经是 Agent 调度的实现，可对标 agentcontrolplane。
...
- AgentGuard 的 `SandboxType` 枚举已覆盖这些场景

## 五、AgentGuard 已有 vs 需要扩展
...
### 需要扩展（增量改进）
1. **对接 A2A 协议标准** — 让 AgentGuard Agent 能与外部 Agent 互操作
...
3. **长期**（未来版本）:
   - Agent 自主调度（AgentGuard 自己决定用哪个 Agent）
   - Agent 学习（从历史任务中优化匹配）
