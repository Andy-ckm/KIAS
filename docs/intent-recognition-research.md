...
| # | 论文 | 年份 | 引用 | 核心贡献 |
|---|------|------|------|----------|
| 1 | **A Survey on Large Language Model based Autonomous Agents** | 2024 | 1039 | 系统综述LLM Agent架构：Profile→Memory→Planning→Action四模块，KIAS的IntentRecognizer属于Planning的意图识别子模块 |
...
| # | 论文 | 年份 | 引用 | 核心贡献 |
|---|------|------|------|----------|
| 4 | **Self-Instruct: Aligning Language Models with Self-Generated Instructions** (Wang et al.) | 2023 | 545 | ACL 2023. 自生成指令微调，KIAS的IntentRecognizer可扩展为LLM-based意图分类 |
...
| # | 论文 | 年份 | 引用 | 核心贡献 |
|---|------|------|------|----------|
| 7 | **HuggingGPT: Solving AI Tasks with ChatGPT and its Friends in Hugging Face** (Shen et al.) | 2023 | 264 | LLM作为控制器，将用户意图拆解为子任务，分配给不同专家模型。**直接参考**：KIAS的TaskDecomposer借鉴了其任务拆解+分配架构 |
| 8 | **Tree of Thoughts: Deliberate Problem Solving with Large Language Models** (Yao et al.) | 2023 | 564 | 将复杂问题拆解为树状思维路径，支持回溯和探索。KIAS的DAG任务图借鉴了其分支-合并思想 |
| 9 | **Graph of Thoughts: Solving Elaborate Problems with LLMs** (Besta et al.) | 2024 | 389 | AAAI 2024. 图状任务分解，支持任意依赖关系。**直接参考**：KIAS的TaskGraph拓扑排序借鉴了其图结构 |
| 10 | **ChatDev: Communicative Agents for Software Development** (Qian et al.) | 2024 | 213 | ACL 2024. 多Agent协作开发，通过Chat Chain将软件开发拆解为原子任务。**直接参考**：KIAS的任务模板设计 |
...
| # | 论文 | 年份 | 引用 | 核心贡献 |
|---|------|------|------|----------|
| 12 | **Toolformer: Language Models Can Teach Themselves to Use Tools** (Schick et al.) | 2023 | 1200+ | LLM自主学习何时调用工具。**直接参考**：KIAS的IntentRecognizer可扩展为tool-aware意图识别 |
