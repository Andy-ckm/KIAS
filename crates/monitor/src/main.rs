use kias_monitor::{TelemetryCollector, MetricsCollector};
use kias_monitor::telemetry::{TelemetryEvent, EventType, Severity};
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Latency P50: {:.2}ms", metrics.histogram_percentile("task_latency", 50.0));
    println!("\nPrometheus export:\n{}", metrics.export_prometheus());
    
    let stats = telemetry.stats();
    println!("\nEvent stats: {} events, error rate: {:.2}%", stats.total_events, stats.error_rate * 100.0);
