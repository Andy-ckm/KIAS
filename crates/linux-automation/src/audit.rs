...
/// 获取审计日志
pub fn get_audit_log(db_path: &Path, limit: Option<usize>) -> Result<Vec<AuditEntry>> {
    let conn = Connection::open(db_path)?;
...
/// 获取审计统计
pub fn get_statistics(db_path: &Path) -> Result<AuditStatistics> {
    let conn = Connection::open(db_path)?;
...
mod tests {
    ...
    fn test_record_audit() {
        ...
        record_task_execution(&db_path, &task, &result).unwrap();

        let stats = get_statistics(&db_path).unwrap();
        assert_eq!(stats.total_entries, 1);

        let log = get_audit_log(&db_path, Some(10)).unwrap();
        assert_eq!(log.len(), 1);
