//! Bounded intent classification utilities used by the management API.
//!
//! This module intentionally does not execute, mutate, or recursively improve
//! code. It preserves the existing intent/decomposition response contract while
//! keeping the Core API independent from the experimental `auto-loop` crate.
...
pub mod task_decomposer {
    ...
    fn templates_for(intent: &IntentType) -> &'static [Template] {
        const IMPLEMENT: &[Template] = &[
            ("Analyze requirements", "Define scope and interfaces", 60, &["analysis"]),
            ("Implement", "Create the bounded implementation", 180, &["coding"]),
            ("Verify", "Run deterministic tests and review", 120, &["testing", "review"]),
        ];
        const FIX: &[Template] = &[
            ("Reproduce", "Capture the failing behavior", 60, &["debugging"]),
            ("Diagnose", "Identify the root cause", 90, &["analysis"]),
            ("Repair", "Implement the smallest safe fix", 120, &["coding"]),
            ("Regress", "Run focused and regression tests", 90, &["testing"]),
        ];
        const REVIEW: &[Template] = &[
            ("Inspect", "Run static and structural analysis", 60, &["analysis"]),
            ("Review", "Evaluate correctness, security, and maintainability", 120, &["review"]),
            ("Report", "Produce prioritized findings", 60, &["documentation"]),
        ];
        const OPERATE: &[Template] = &[
            ("Assess", "Validate desired state and constraints", 60, &["operations"]),
            ("Plan", "Prepare a reversible execution plan", 90, &["analysis"]),
            ("Verify", "Define health and rollback checks", 60, &["testing"]),
        ];
        const GENERIC: &[Template] = &[
            ("Clarify", "Clarify the requested outcome and constraints", 60, &["analysis"]),
...
mod tests {
    ...
    fn classifier_is_bounded_and_deterministic() {
        ...
        assert_eq!(format!("{:?}", first.intent_type), format!("{:?}", second.intent_type));
