...
export default function SchedulerPage() {
  ...
  const nodeBarData = data.node_utilization.map(node => ({
    name: node.node_name.length > 10 ? `${node.node_name.slice(0, 10)}…` : node.node_name,
    agents: node.agent_count,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Success Rate"
          value={formatPercent(data.throughput.success_rate)}
          icon="✅"
          color="blue"
        />
        <StatCard
          label="Avg Restarts"
          value={data.throughput.avg_restart_count.toFixed(1)}
          icon="🔄"
          color="purple"
        />
      </div>
                ...
                <Legend
                  formatter={(value: string) => (
                    <span style={{ color: '#94a3b8' }}>
                      {value === 'agents' ? 'Total Agents' : 'Running'}
                    </span>
                  )}
                />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-purple-400">
              {formatPercent(data.throughput.success_rate)}
            </p>
          ...
          <tbody className="divide-y divide-slate-700/50">
            {data.recent_decisions.map(decision => (
              <tr key={decision.agent_id} className="hover:bg-slate-700/20 transition-colors">
                ...
                <td className="px-5 py-3">
                  <span
                    className={`text-sm font-medium ${PRIORITY_COLORS[decision.priority] || 'text-slate-400'}`}
                  >
                    {decision.priority}
                  </span>
                </td>
