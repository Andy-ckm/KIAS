...
const DETECTION_RULES: &[DetectionRule] = &[
    ...
    DetectionRule {
        ...
        pattern: r"(?i)(send|post|upload|transmit|forward)\s+(this|all|the)\s+(the\s+)?(data|info|context|prompt|conversation)\s+(to|via|through)",
        severity: InjectionSeverity::Critical,
    ...
    DetectionRule {
        name: "tool_abuse",
        pattern: r"(?i)(call|invoke|execute|use|run)\s+(the\s+)?(tool|function|api|command|shell|bash|exec)\s+(\w+\s+)?(to|for|that)",
        severity: InjectionSeverity::Medium,
