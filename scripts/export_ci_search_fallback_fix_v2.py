#!/usr/bin/env python3
"""Structurally replace SearchTool with a deterministic built-in regex engine."""
...
def main() -> None:
    ...
    path = Path("crates/tool-executor/src/builtin.rs")
    text = path.read_text(encoding="utf-8")
    pattern = re.compile(
...
fn search_file(
    ...
    let initial_match_count = outcome.matches.len();
    ...
    loop {
        ...
        // NUL bytes are a low-cost signal that this is not a text file. Remove
        // any earlier matches from the same file before classifying it as binary.
        if buffer.contains(&0) {
            outcome.matches.truncate(initial_match_count);
...
fn search_path(
    ...
    if metadata.is_file() {
        if path_matches_glob(path, glob) {
            search_file(path, regex, limit, outcome)?;
        }
        return Ok(());
    }
...
'''
    ...
    if count != 1:
        raise SystemExit(f"SearchTool structural replacement: expected one block, found {count}")
    text = text.replace(
    ...
    Path("scripts/export_ci_search_fallback_fix.py").unlink(missing_ok=True)
    Path("scripts/export_ci_search_fallback_fix_v2.py").unlink()
